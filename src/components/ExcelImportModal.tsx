      ...
      <div className="relative bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md p-6">
        ...
        {step === 'warning' && warning && (
          <div className="space-y-4">
            ...
            {warning.mismatches && warning.mismatches.length > 0 && (
                ...
                <div className="flex items-start gap-3 mb-2">
                  ...
                  <div>
                    <p className="text-red-400 font-semibold text-sm">
                      {warning.mismatches.length} viaje{warning.mismatches.length === 1 ? '' : 's'} con placa que no coincide con el conductor
                    </p>
                    <p className="text-red-300/80 text-xs mt-1">
                      El archivo trae un conductor distinto al chofer asignado de esa placa — probablemente la romana escribió la placa de otro camión parecido. Revísalos después de importar en Viajes.
                    </p>
                  </div>
                </div>
                <div className="max-h-40 overflow-y-auto space-y-1.5 mt-3">
                  {warning.mismatches.map((m, i) => (
                    <div key={i} className="bg-zinc-900/60 rounded-lg px-3 py-2 text-xs">
                      <p className="text-zinc-300">Ticket <span className="font-mono">{m.ticketNo}</span> · {m.date} · placa <span className="font-mono">{m.plate}</span></p>
                      <p className="text-zinc-500 mt-0.5">
                        Archivo dice <span className="text-red-400 font-medium">{m.driverInFile}</span>, pero {m.plate} está asignada a <span className="text-emerald-400 font-medium">{m.assignedDriver}</span>
                      </p>
                    </div>
                  ))}
                </div>
