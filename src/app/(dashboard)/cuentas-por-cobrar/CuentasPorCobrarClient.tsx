          ...
          <form onSubmit={editing ? handleUpdate : handleCreate} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input name="clientName" required defaultValue={editingData?.clientName ?? ''}
              ...
              <div>
                ...
                <input name="date" type="date" required defaultValue={editingData ? editingData.date.slice(0, 10) : today}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input name="concept" required defaultValue={editingData?.concept ?? ''}
                  placeholder="Ej: Relación nº3 — transporte minero"
                  className="w-full bg-zinc-800 border border-zinc-700 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-amber-500 placeholder:text-zinc-600" />
              </div>
              <div>
                <label className="block text-xs text-zinc-400 mb-1.5">Período</label>
                <input name="periodLabel" defaultValue={editingData?.periodLabel ?? ''}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input name="totalAmount" type="number" step="0.01" min="0.01" required
                  defaultValue={editingData?.totalAmount ?? ''}
                  placeholder="0.00"
                  className="w-full bg-zinc-800 border border-zinc-700 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-amber-500 placeholder:text-zinc-600" />
              </div>
              <div>
                ...
                <input name="amountPaid" type="number" step="0.01" min="0"
                  defaultValue={editingData?.amountPaid ?? '0'}
                  placeholder="0.00"
                  className="w-full bg-zinc-800 border border-zinc-700 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-amber-500 placeholder:text-zinc-600" />
              </div>
            </div>
            <div>
              ...
              <input name="notes" defaultValue={editingData?.notes ?? ''}
        ...
        <div className="bg-zinc-900 border border-blue-500/20 rounded-2xl p-5">
          ...
          <form onSubmit={handlePayment} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs text-zinc-400 mb-1.5">Notas</label>
                <input name="notes" placeholder="Referencia, observación..."
                  className="w-full bg-zinc-800 border border-zinc-700 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-amber-500 placeholder:text-zinc-600" />
              </div>
            </div>
            {error && <p className="text-red-400 text-sm">{error}</p>}
                ...
                {filtered.map(c => (
                  <React.Fragment key={c.id}>
                    ...
                    {expanded === c.id && c.payments.length > 0 && (
                      <tr key={`${c.id}-payments`} className="bg-zinc-800/30 border-b border-zinc-800/50">
