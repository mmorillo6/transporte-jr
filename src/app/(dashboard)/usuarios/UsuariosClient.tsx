    ...
    <div className="space-y-4">
      ...
      {(showForm || editing) && (
        <div className="bg-zinc-900 border border-amber-500/20 rounded-2xl p-5">
          <h3 className="text-white font-semibold mb-4">{editing ? 'Editar usuario' : 'Nuevo usuario'}</h3>
              ...
              <div>
                ...
                <div className="relative">
                  <input name={editing ? 'newPassword' : 'password'} type={showPassword ? 'text' : 'password'}
                    ...
                    className="w-full bg-zinc-800 border border-zinc-700 text-white rounded-xl px-3 py-2.5 pr-10 text-sm focus:outline-none focus:border-amber-500 placeholder:text-zinc-600" />
        ...
        {filtered.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map(user => (
                  <tr key={user.id} className={`border-b border-zinc-800/50 hover:bg-zinc-800/20 transition-colors ${!user.active ? 'opacity-50' : ''}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <button onClick={() => { setEditing(user); setShowForm(false); setError('') }}
